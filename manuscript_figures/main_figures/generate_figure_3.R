...
death_threshold_specific_corr <- function(death_threshold, MIN_countries) {
  ...
  # adjust the p values as a function of the number of days for each death threshold
  melted_df <- melted_df %>%
    group_by(source) %>%
    mutate(p_value_c = p.adjust(p_value, method = "fdr"))

  melted_df$sig[which(melted_df$p_value_c <= .05)] <- 1
...
death_plot <- ggplot(norm_df_Death, aes(days, correlation, group = confirmed)) +
  ...
  ylab(TeX("EMP score-deaths per million correlation ($\\rho$)")) +
  xlab("normalized days")

# ggsave(death_plot, filename = paste0(Ensemble_PATH, "/plots/main_figures/Figure_3A_corrected_p.pdf"), width = 14.22, height = 5.33)

# create plot of just day 50 figure 3B
death_thres <- 50

cor_set_and_p_val_for_corr_of_rank_scores <- t(sapply(1:22, function(w) {
  day <- w
  ...
  c(cor.test(dd$struct_prot_rank, dd$death_rank)$estimate, pvalue = cor.test(dd$struct_prot_rank, dd$death_rank)$p.value)
})) %>% data.frame()
...
day_point_plots <- lapply(1:5, function(w) {
  ...
  plot <- dd %>% ggplot(aes(x = death_rank, y = struct_prot_rank, label = country)) +
    geom_smooth(method = "lm", color = "black") +
    geom_text_repel() +
    geom_point(size = 2, aes(color = group)) +
    ggtitle(paste("day", day, sep = " ")) +
    theme_classic() +
    xlab("death rate rank") +
    ylab("EnsembleMHC population rank") +
    scale_x_continuous(breaks = seq(2, max_Deathrank, 2)) +
    scale_y_continuous(breaks = seq(2, max_EMPrank, 2)) +
    theme(legend.position = "none") +
    geom_hline(yintercept = median(dd$struct_prot_rank), linetype = "dashed")
})
...
# calculate the p-values of each day at the threshold so p-values can be adjusted
death_thres <- 50
p_values_for_boxes <- sapply(1:22, function(day) {
  day_point <- get_day_data(day, death_thres, MIN_countries = 8)
  ...
  dd <- day_point %>%
    ...
    arrange(death_per_pop) %>%
    mutate(death_rank = 1:length(death_per_pop)) %>%
    mutate(group = if_else(struct_prot_rank > median(struct_prot_rank), "upper half", "lower half"))
...
day_point_boxes <- lapply(c(1, 6, 12, 17, 22), function(day) {
  ...
  dd %>% ggplot(aes(x = group, y = death_per_pop, fill = group)) +
    geom_boxplot() +
    xlab("EnsembleMHC population rank") +
    ylab("deaths per 1M") +
    theme_pubclean() +
    theme(legend.position = "none") +
    ggtitle(paste("day", day, sep = " "))
