...
# get peptide pos
pep_pos <- all_raw %>%
  slice(which(peptide %in% all_filtered$peptide)) %>%
  select(peptide, pos, gene)
# all repeated peptides
prot_peps <- filtered_peps %>%
  merge(pep_pos) %>%
  select(peptide, gene, pos, HLA) %>%
  unique()
...
align_plots <- lapply(1:length(prot_aligned), function(j) {
  ...
  ggplot(df, aes(x = pos, y = count)) +
    geom_line(color = cols_prot[j]) +
    theme_calc() +
...
poly_plots <- lapply(1:length(seqs), function(j) {
  i <- seqs[[j]]
  df <- data.frame(pos = 1:nrow(i), poly = apply(i, 1, function(w) {
    length(table(w))
  }))
  ...
  ggplot(df, aes(x = pos, y = poly.1 - 1)) +
    geom_line(stat = "identity", color = cols_prot[j]) +
    ylab("polymorphisms") +
    xlab("positions") +
    theme_pubclean() +
