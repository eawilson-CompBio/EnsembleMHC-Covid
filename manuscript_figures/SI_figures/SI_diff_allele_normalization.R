...
allele_freq_list <- lapply(test_set, function(w) {
  ...
  if (length(total_n) > 0) {
    tmp <- w %>%
      mutate(eff_pop = allele_freq * n) %>%
      replace_na(list(eff_pop = 0)) %>%
      group_by(HLA) %>%
      summarise(pop_freq = sum(eff_pop) / total_n) %>%
      mutate(country = name) %>%
...
death_threshold_iter <- mclapply(1:100, function(death_thres) {
  death_threshold_specific_corr(death_thres, 6)
}, mc.cores = 10)
...
# plot results
death_plot_all_alleles_mean <- ggplot(norm_df_Death, aes(days, correlation, group = confirmed)) +
  ...
  ggtitle("Sample-weighted mean of allele frequencies")
...
allele_freq_list <- lapply(test_set, function(w) {
  ...
  if (length(total_n) > 0) {
    tmp <- w %>%
      mutate(eff_pop = allele_freq * n * 2) %>%
      replace_na(list(eff_pop = 0)) %>%
      group_by(HLA) %>%
      summarise(allele_count = sum(eff_pop)) %>%
      mutate(pop_freq = allele_count / sum(allele_count)) %>%
      mutate(country = name) %>%
      mutate(total_n = total_n) %>%
      slice(which(HLA %in% sel_HLA))
  } else {
...
death_plot_all_alleles_relative <- ggplot(norm_df_Death, aes(days, correlation, group = confirmed)) +
  geom_line(aes(color = confirmed)) +
  scale_color_viridis() +
  geom_point(data = sig, aes(days, correlation), color = "red", size = .25) +
  facet_wrap(source ~ .) +
  ...
  ylab(TeX("correlation ($\\rho$)")) +
  xlab("normalized days") +
  ggtitle("Normalization with respect to EnsembleMHC alleles")
