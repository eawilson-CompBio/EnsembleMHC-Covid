...
death_plot <- ggplot(norm_df_Death, aes(days, correlation, group = confirmed)) +
  ...
  labs(color = "number of deaths at day 0") +
...
df <- df_spearman_pwr %>%
  select(source, sig, PPV) %>%
  group_by(source) %>%
  summarise(prop_p = table(sig)[2] / sum(table(sig)), prop_ppv = table(PPV >= .95)[2] / sum(table(PPV >= .95))) %>%
  melt()

# plot data
plot <- ggplot(df, aes(x = variable, y = value)) +
