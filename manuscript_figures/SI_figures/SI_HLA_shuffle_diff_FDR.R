...
scramble_test[[c]]<-data.frame(norm_df_Death,peptideFDR=i)
c=c+1
}

all_combined_scramble<-do.call(rbind,lapply(scramble_test,function(i){
  i%>%group_by(source,confirmed,peptideFDR)%>%mutate(correct_p = p.adjust(p_value,method="fdr"))
}))



p1<-all_combined_scramble %>%
  ungroup() %>%
  slice(which(source == "SARS-CoV-2 structural proteins")) %>%
  select(correlation, p_value, peptideFDR) %>%
  reshape2::melt(id.vars = c("peptideFDR")) %>%
  group_by(peptideFDR, variable) %>%
  mutate(up = quantile(value, .75), down = quantile(value, .25),med=median(value)) %>%
  ggplot(aes(x = peptideFDR)) +
  geom_line(aes(y=med)) +
  geom_ribbon(aes(ymin = down, ymax = up, fill = variable), alpha = .25) +
  facet_wrap(variable ~ ., nrow = 2) + ggtitle("SARS-CoV-2 structural proteins")


p2<-all_combined_scramble %>%
  ungroup() %>%
  slice(which(source == "entire SARS-CoV-2 proteome")) %>%
  select(correlation, p_value, peptideFDR) %>%
  reshape2::melt(id.vars = c("peptideFDR")) %>%
  group_by(peptideFDR, variable) %>%
  mutate(up = quantile(value, .75), down = quantile(value, .25),med=median(value)) %>%
  ...
  geom_line(aes(y=med)) +
  geom_ribbon(aes(ymin = down, ymax = up, fill = variable), alpha = .25) +
  facet_wrap(variable ~ ., nrow = 2) + ggtitle("all SARS-CoV-2 proteins")

library(patchwork)
garg<-p1/p2

ggsave(garg,filename = paste0(pdf_dir,"HLA_scramble_diff_fdr.pdf"),width = pt2in(1024),height = pt2in(768))