...
#isolate only structural proteins 
norm_df_Death<-norm_df_Death[which(norm_df_Death$source=="SARS-CoV-2 structural proteins"),]
sig<-sig[which(sig$source=="SARS-CoV-2 structural proteins"),]
...
death_plot<-ggplot(norm_df_Death,aes(days,correlation,group=confirmed))+geom_line(aes(color=confirmed))+
  scale_color_viridis()+
  geom_point(data = sig,aes(days,correlation),color="red",size=1)+
  theme_linedraw()+geom_line(data = norm_df_Death[which(norm_df_Death$confirmed==50),],aes(days,correlation),fill="orange",size=2)+
  theme(legend.position = "top")+labs(color="number of deaths at day 0")+guides(colour=guide_colorbar(title.position = "right"))+ylab(TeX("EMP score-deaths per million correlation ($\\rho$)"))+
...
ggsave(death_plot,filename = paste0(Ensemble_PATH,"/plots/main_figures/figure_3A_test.pdf"),width = 14.11,height = 5.33)
...
day_point_plots<-lapply(1:5,function(w){
  ...
  #create linear model
print(cor.test(dd$death_per_pop,dd$Structural.proteins,method="spearman"))
  dd$half<-if_else(dd$struct_prot_rank>median(dd$struct_prot_rank),"upper half","lower half")
  P<-dd%>%ggplot(aes(death_rank,struct_prot_rank,label=country))+geom_text_repel()+geom_point(aes(color=half),size=2)+
...
ggsave(corrs,filename =paste0(Ensemble_PATH,"/plots/main_figures/figure_3B_no_fill_test.pdf"),width = 14.11,height = 2.5)
...
day_point_boxes<-lapply(c(1,6,12,17,22),function(day){
  ...
  #this was added to graphs in key not becasue the text was difficut to see. You can verify the p vaue by un commenting the following ines
  ...
  dd%>%ggplot(aes(x=group,y=death_per_pop,fill=group))+geom_boxplot()+
    #annotate(geom = "text",x = 1.5,y=.8*max(dd$death_per_pop),label=paste("p =", wt))+
...
ggsave(boxes,filename =paste0(Ensemble_PATH,"/plots/main_figures/figure_3C_test.pdf"),width = 14.11,height = 2.5)
