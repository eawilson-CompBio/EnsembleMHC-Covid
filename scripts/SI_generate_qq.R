...
all_big<-do.call(rbind,allele_freq_list)
  ...
  day_data<-CoV_data_sel_countries%>%slice(which(Deaths>=1))%>%
  ...
  score_and_death_pop<-score_and_death_pop%>%group_by(country,days)%>%mutate(popM=`2020`/1000)%>%mutate(death_per_pop=Deaths/popM)%>%merge(pop_big)%>%select(country,death_per_pop,Deaths,days,All.proteins,Structural.proteins)
