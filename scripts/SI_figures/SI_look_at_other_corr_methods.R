...
all_HLA_data<-fread("~/Covid-19/seq_data/HLA_freqs_2020_update.csv",stringsAsFactors = F,header = T)
#countries with at least 1 case reported case by 04/10
...
death_threshold_specific_corr<-function(death_threshold,MIN_countries,method){
  #filter at a set number of deaths
  #set days for time series analysis
  #add days since it reached that point
  day_data<-CoV_data_sel_countries%>%slice(which(Deaths>=death_threshold))%>%
  ...
  cor_data<-lapply(1:thres,function(i){
    #create subset of data at day i
    tmp<-score_and_death_pop%>%slice(which(days==i))
    list(cor_struct=cor(tmp$Structural.proteins,tmp$death_per_pop,method = method),
         cor_all_prot=cor(tmp$All.proteins,tmp$death_per_pop,method = method),
         tab=tmp,
         day=w,
         p_struct=cor.test(tmp$Structural.proteins,tmp$death_per_pop,method = method)[[3]],
         p_all_prot=cor.test(tmp$All.proteins,tmp$death_per_pop,method = method)[[3]])
...
death_threshold_iter_spearman<-mclapply(1:100,function(death_thres){death_threshold_specific_corr(death_thres,8,"spearman")},mc.cores = 10)
...
death_plot_spearman<-ggplot(norm_df_Death,aes(days,correlation,group=confirmed))+geom_line(aes(color=confirmed))+
  scale_color_viridis()+
  ...
  xlab("normalized days")+ggtitle("Spearman")


death_threshold_iter_pearson<-mclapply(1:100,function(death_thres){death_threshold_specific_corr(death_thres,8,"pearson")},mc.cores = 10)
...
death_plot_pearson<-ggplot(norm_df_Death,aes(days,correlation,group=confirmed))+geom_line(aes(color=confirmed))+
  scale_color_viridis()+
  geom_point(data = sig,aes(days,correlation),color="red",size=.25)+
  facet_wrap(source~.)+theme_linedraw()+theme(legend.position = "bottom")+labs(color="number of deaths by day 0")+guides(colour=guide_colorbar(title.position = "bottom"))+ylab(TeX("correlation ($\\rho$)"))+
  xlab("normalized days")+ggtitle("Pearson")


death_threshold_iter_kendall<-mclapply(1:100,function(death_thres){death_threshold_specific_corr(death_thres,8,"kendall")},mc.cores = 10)
...
death_plot_kendall<-ggplot(norm_df_Death,aes(days,correlation,group=confirmed))+geom_line(aes(color=confirmed))+
  scale_color_viridis()+
  geom_point(data = sig,aes(days,correlation),color="red",size=.25)+
  facet_wrap(source~.)+theme_linedraw()+theme(legend.position = "bottom")+labs(color="number of deaths by day 0")+guides(colour=guide_colorbar(title.position = "bottom"))+ylab(TeX("correlation ($\\rho$)"))+
  xlab("normalized days")+ggtitle("Kendall")

df_spearman_pwr$pwr<-apply(df_spearman_pwr,1,function(w){
      pwr.r.test(r = as.numeric(w[3]),n = as.numeric(w[6]),sig.level = .05)[[4]]
})

spearman_pwr<-df_spearman_pwr%>%ggplot(aes(x=days,y=pwr,group=confirmed))+geom_line(aes(color=confirmed))+
  scale_color_viridis()+
  geom_point(data = subset(df_spearman_pwr,sig==1),aes(days,pwr),color="red",size=.25)+
  facet_wrap(source~.)+theme_linedraw()+theme(legend.position = "bottom")+labs(color="number of deaths by day 0")+guides(colour=guide_colorbar(title.position = "bottom"))+
  ylab("power")+geom_hline(yintercept = .8,color="orange")+
  ggtitle("spearman")
...
pearson_pwr<-df_pearson_pwr%>%ggplot(aes(x=days,y=pwr,group=confirmed))+geom_line(aes(color=confirmed))+
  scale_color_viridis()+
...
kendall_pwr<-df_kendall_pwr%>%ggplot(aes(x=days,y=pwr,group=confirmed))+geom_line(aes(color=confirmed))+
  scale_color_viridis()+
...
deathplots<-death_plot_pearson + death_plot_spearman + death_plot_kendall + plot_layout(ncol = 1,guides = "collect")+ 
  plot_annotation(title = "population EsembleMHC score and death rate correlation",subtitle = paste("countries with at least",confirmed_case_threshold,"cases",sep = " "))
pwrplots<-pearson_pwr + spearman_pwr + kendall_pwr + plot_layout(ncol = 1,guides = "collect")+ 
  plot_annotation(title = "population EsembleMHC score and death rate power ",subtitle = paste("countries with at least",confirmed_case_threshold,"cases",sep = " "))


ggsave(deathplots,filename = paste("~/Covid-19/plots/SI_figures/SI_deathplots_by_corr_methods_cases_",confirmed_case_threshold,".pdf",sep = ""),height = 10,width = 10)
ggsave(pwrplots,filename = paste("~/Covid-19/plots/SI_figures/SI_pwrplots_by_corr_methods_cases_",confirmed_case_threshold,".pdf",sep = ""),height = 10,width = 10)
#ggsave(powered_dist,filename = paste("~/Covid-19/plots/SI_figures/SI_pwr_dist_by_corr_methods_cases_",confirmed_case_threshold,".pdf",sep = ""),height = 10,width = 10)
