...
all_proteins <- fread(paste0(dataset_path,"all_peptides_prefilter.csv"))
p1 <- all_proteins%>%ggplot(aes(x=prob))+geom_density()+geom_vline(xintercept = .05,color="red")+theme_pubclean()+xlab("peptide FDR")
...
p2 <- data.frame(len=as.numeric(names(struct_tab)),count=as.numeric(struct_tab)/sum(struct_tab))%>%
  ggplot(aes(len,count))+geom_bar(stat="identity",fill="#00ACED")+xlab("peptide length")+ylab("% of peptides")+
  ggtitle("SARS-CoV-2 structural proteins")+theme_pubclean()+scale_x_continuous(breaks = 8:14)

p3 <- data.frame(len=as.numeric(names(all_tab)),count=as.numeric(all_tab)/sum(all_tab))%>%
  ggplot(aes(len,count))+geom_bar(stat="identity",fill="#ED0036")+xlab("peptide length")+ylab("% of peptides")+
  ggtitle("full SARS-CoV-2 proteome")+theme_pubclean()+scale_x_continuous(breaks = 8:14)
