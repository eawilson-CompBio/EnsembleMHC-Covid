...
get_stats<-function(death_threshold,MIN_countries){
  ...
  cor_data<-lapply(1:thres,function(i){
    ...
    list(spearmanConfInt(tmp$Structural.proteins,tmp$death_per_pop),
         spearmanConfInt(tmp$All.proteins,tmp$death_per_pop),num_of_country=nrow(tmp))
    
  })
  
data.frame(rbind(data.frame(prot_group="structural",t(sapply(cor_data,function(s)s[[1]])),day=1:thres),
      data.frame(prot_group="full",t(sapply(cor_data,function(s)s[[2]])),day=1:thres)),death_threshold=death_threshold,Countries=sapply(cor_data,function(s)s[[3]]))
...
spearmanConfInt<-function (x, y, level = 0.95){
  ...
  for (i in 1:n) Z[i] <- n * U - (n - 1) * cor(x[-i], y[-i],method = "spearman")
    ...
    l <- uniroot(g, interval = c(-1, U), tol = 0.1 * 10^{-10},extendInt = "yes")$root
    if(l< -1){l= -1}
    ...
    u <- uniroot(g, interval = c(U, 1), tol = 0.1 * 10^{-10})$root
...
country_count<-all_s%>%ggplot(aes(x=day,y = Countries))+geom_line()+theme_classic()+
  facet_wrap(death_threshold~.)+xlab("days from death threshold")+ylab("number of countries")+
