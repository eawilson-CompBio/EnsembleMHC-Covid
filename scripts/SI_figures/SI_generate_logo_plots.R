...
peps<-all_counts%>%slice(which(prob<=.05))%>%group_split(HLA)
...
peps<-lapply(peps,function(P){
  ...
  if(nchar(H)>9){paste(str_split(H,"",simplify = T)[c(1:8,nchar(H))],collapse = "")
    }else if(nchar(H)<9){ tmp<-str_split(H,"",simplify = T)
