...
allele_freq_list<-lapply(test_set,function(w){
  ...
  #all allele mean
       if(length(total_n)>0){
    tmp<-w%>%mutate(eff_pop=allele_freq*n)%>%replace_na(list(eff_pop=0))%>%group_by(HLA)%>%summarise(pop_freq=sum(eff_pop)/total_n)%>%mutate(country=name)%>%mutate(total_n=total_n)%>%slice(which(HLA%in%sel_HLA))
   }else{data.frame()}
...
#plot results
death_plot_all_alleles_mean<-ggplot(norm_df_Death,aes(days,correlation,group=confirmed))+geom_line(aes(color=confirmed))+
  scale_color_viridis()+
  ...
  xlab("normalized days")+ggtitle("Sample-weighted mean of allele frequencies")

#-------------------------------------------
#normalization with respect to all alleles 
#-------------------------------------------
...
allele_freq_list<-lapply(test_set,function(w){
  ...
  #all alleles relative
      if(length(total_n)>0){
   tmp<-w%>%mutate(eff_pop=allele_freq*n*2)%>%replace_na(list(eff_pop=0))%>%group_by(HLA)%>%summarise(allele_count=sum(eff_pop))%>%mutate(pop_freq=allele_count/sum(allele_count))%>%mutate(country=name)%>%mutate(total_n=total_n)%>%slice(which(HLA%in%sel_HLA))
      }else{data.frame()}
...
death_plot_all_alleles_relative<-ggplot(norm_df_Death,aes(days,correlation,group=confirmed))+geom_line(aes(color=confirmed))+
  scale_color_viridis()+
  geom_point(data = sig,aes(days,correlation),color="red",size=.25)+
  facet_wrap(source~.)+theme_linedraw()+theme(legend.position = "bottom")+labs(color="number of deaths by day 0")+guides(colour=guide_colorbar(title.position = "bottom"))+ylab(TeX("correlation ($\\rho$)"))+
  xlab("normalized days")+ggtitle("Normalization with respect to EnsembleMHC alleles")

#---------------------------------------------------------
#normalization with respect to only the 52 select alleles
#---------------------------------------------------------
