...
p1<-data.frame(HLA=factor(df_all_w_std$HLA,levels = rev(df_all_w_std$HLA)),count=all_sample)%>%ggplot(aes(x=HLA,y=count))+
  geom_bar(stat = "identity")+coord_flip()+ylab("number of peptides")+ggtitle("full SARS-CoV-2 proteome: uniform distrubtion")+
  theme_classic()

p2<-data.frame("pep"=all_sample)%>%ggplot(aes(pep))+geom_density()+xlab("number of peptides")+
  theme_classic()
...
p3<-data.frame(HLA=factor(df_all_w_std$HLA,levels = rev(df_all_w_std$HLA)),count=struct_sample)%>%ggplot(aes(x=HLA,y=count))+
  geom_bar(stat = "identity")+coord_flip()+ylab("number of peptides")+ggtitle("SARS-CoV-2 structural proteins: uniform distrubtion")+
  theme_classic()

ks.test(struct_sample,df_struct_w_std$total_count_struct)
p4<-data.frame("pep"=struct_sample)%>%ggplot(aes(pep))+geom_density()+xlab("number of peptides")+scale_x_continuous(breaks = seq(0,max(struct_sample),1))+
  theme_classic()
...
ks.test(df_all_w_std$total_count_all_proteins,df_struct_w_std$total_count_struct)
ks.test(round(rnorm(n = 52,median(df_all_w_std$total_count_all_proteins),sd = 3)),df_all_w_std$total_count_all_proteins)
