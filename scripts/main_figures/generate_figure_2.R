...
p1=df_all_proteins%>%ggplot(aes(x=factor(HLA,levels = rev(unique(HLA))),y=count,fill=gene))+
  geom_bar(stat = "identity")+coord_flip()+ggtitle("full SARS-CoV-2 protome")+scale_fill_manual(values = c25[1:10])+
  theme_linedraw()+theme(plot.title = element_text(hjust = 0.5))+ylab("number of peptides")+xlab("HLA")

# create another bar plot of only structural proteins
df_struct<-all_counts%>%slice(which(prob<=prob_threshold))%>%slice(which(gene%in%c("E","N","M","S")))%>%group_by(HLA,gene)%>%summarise(count=length(gene))
...
p2=df_struct%>%ggplot(aes(x=HLA,y=count,fill=gene))+geom_bar(stat = "identity")+coord_flip()+ggtitle("SARS-CoV-2 structural proteins")+scale_fill_manual(values = c25[c(1:3,10)])+theme_linedraw()+theme(axis.text.y = element_blank(),axis.title.y = element_blank(),axis.ticks.y = element_blank())+theme(plot.title = element_text(hjust = 0.5))+ylab("number of peptides")+scale_y_continuous(breaks = seq(0,12,2))



#calculate the standev of each allele with respect to all proteins 
df_all_w_std<-df_all_proteins%>%group_by(HLA)%>%
  summarise(total_count_all_proteins=sum(count))%>%mutate(std_all=std(total_count_all_proteins))%>%
...
df_struct_w_std<-df_struct%>%group_by(HLA)%>%summarise(total_count_struct=sum(count))%>%
...
write.csv(all_counts,paste0(dataset_path,"/all_predictions_aggregated_and_no_FDR_filter.csv"),row.names = F)
#the dataframe of total number of peptide per allele for all proteins
write.csv(df_all_w_std,paste0(dataset_path,"/identified_peptides_all_proteins_summarise_HLA_protein_counts.csv"),row.names = F)
#teh dataframe of total number of peptide per allele for structural proteins
write.csv(df_struct_w_std,paste0(dataset_path,"/identified_peptides_all_structural_proteins_only_summarise_HLA_protein_counts.csv"),row.names = F)

#these are the same as above but retain the protein level information 
write.csv(df_all_proteins,paste0(dataset_path,"/identified_peptides_all_proteins_summarise_w_gene_counts.csv"),row.names = F)
write.csv(df_struct,paste0(dataset_path,"/identified_peptides_all_structural_proteins_only_summarise_HLA_protein_w_gene_counts.csv"),row.names = F)
