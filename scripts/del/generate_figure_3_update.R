...
day_point_plots<-lapply(1:4,function(w){
  ...
  dd<-day_point%>%arrange(Structural.proteins)%>%mutate(struct_prot_rank=1:length(Structural.proteins))%>%arrange(death_per_pop)%>%mutate(death_rank=1:length(death_per_pop))
   ggscatter(dd,x="death_rank",y="struct_prot_rank",label="country",add="reg.line",repel = T)+
     stat_cor(method = "spearman",label.x = max(dd$death_rank)*.65,label.y = max(dd$struct_prot_rank))+
...
ylab("mean power")+xlab("Death threshold")+ggtitle("average statisitical power as a function of death threshold upper limit")+
theme_classic()


data.frame(x=1:1000,mean_pwr=sapply(1:1000,function(n){
  df_spearman_pwr$num_country[which(df_spearman_pwr$confirmed==n)][14]
}))%>%ggplot(aes(x,mean_pwr))+geom_line()+geom_vline(xintercept = 100)+geom_hline(yintercept = 12)+ylab("number of countries at day 7")+
