...
files<-list.files("~/Covid-19/cell_line_data/pred_pep_output/",full.names = T)
...
F1_out_MHC<-do.call(rbind,lapply(quantile(cell_line_data$mhcflurry_presentation_score,c(.98,.99)),function(i){
    thres=i
    print(thres)
    tmp<-cell_line_data%>%data.frame()
    ...
    data.frame(algo="mhcflurry_presentation_score",F1=as.numeric(score["f"]),PPV=as.numeric(score["per"]),recall=as.numeric(score["rec"]),threshold=thres)  

}))
F1_out_MHC$qual<-"high"
F1_out_MHC$qual[which.max(F1_out_MHC$threshold)]<-"low"
...
p_b_bar<-all_out%>%dplyr::select(algo,precision,recall,threshold,qual)%>%group_by(algo,qual,threshold)%>%mutate(precision=mean(precision,na.rm=T),recall=mean(recall,na.rm=T))%>%
    melt(id.vars=c("algo","qual","threshold"))%>%ggplot(aes(x=algo,y = value,fill=variable))+geom_bar(stat="identity",position = position_dodge())+
...
all_out%>%ggplot(aes(x=algo,y=F1,fill=as.factor(qual)))+
  geom_bar(stat="identity",position = position_dodge())+theme(axis.text.x = element_text(angle = 90))+facet_wrap(cell_line~.)
all_out%>%ggplot(aes(x=algo,y=PPV,fill=as.factor(qual)))+
  geom_bar(stat="identity",position = position_dodge())+theme(axis.text.x = element_text(angle = 90))+facet_wrap(cell_line~.)


df%>%ggplot(aes(x=algo,y=PPV,fill=as.factor(qual)))+geom_bar(stat="identity",position = position_dodge())

F1(df_all_proteins$ident,pos)
