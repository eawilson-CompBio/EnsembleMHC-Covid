...
true_counts<-cell_line_data%>%slice(-which(duplicated(peptide)))%>%pull(ident)%>%table()
...
pep_probs<-lapply(unique(cell_line_data$HLA),function(w){
  ...
  tmp <- cell_line_data%>%dplyr::slice(which(cell_line_data$HLA==w))
  ...
  prob_combo<-do.call(rbind,prob_list)%>%data.frame()%>%
    group_by(peptide)%>%summarise(prob=prod(prob))%>%merge(tmp[,c("peptide","HLA","ident")])
...
F1_out_MHC<-do.call(rbind,lapply(c(2,.5),function(i){
    ...
    }))%>%arrange(desc(mhcflurry_presentation_score))%>%slice(-which(duplicated(peptide)))
...
F1_ensemble<-do.call(rbind,lapply(c(.5,.05),function(i){
  prob_threshold<-i
  df_all_proteins<-all_counts%>%slice(which(prob<=prob_threshold))%>%slice(-which(duplicated(peptide)))
  data.frame(algo="EnsembleMHC",
...
p_b_bar<-all_out%>%dplyr::select(algo,precision,recall,threshold,qual)%>%group_by(algo,qual,threshold)%>%mutate(precision=mean(precision,na.rm=T),recall=mean(recall,na.rm=T))%>%
    melt(id.vars=c("algo","qual","threshold"))%>%unique()%>%ggplot(aes(x=algo,y = value,fill=variable))+geom_bar(stat="identity",position = position_dodge())+
    facet_wrap(qual~.,ncol = 1)+labs(fill="metric")+xlab("")+ylab("")+theme_pubclean()+theme(axis.text.x = element_text(angle = 90))+
  guides(fill=guide_legend(title.position = "top",title.hjust = .5))


hp<-all_out%>%ggplot(aes(y=combo_name,x = cell_line,fill=F1))+geom_tile()+theme_classic()+scale_fill_gradientn(colours = cols)+
  theme(legend.position = "top",axis.text.x = element_text(angle = 90),axis.text.y = element_text(colour = lab_col))+xlab("")+ylab("")+
  guides(fill=guide_colorbar(title.position = "top",title.hjust = .5))

bp<-all_out%>%slice(-which(is.na(F1)))%>%group_by(combo_name)%>%summarise(F1=mean(F1))%>%ggplot(aes(y=combo_name,x = F1))+
  geom_bar(stat="identity")+theme_pubclean()+theme(axis.text.y = element_blank())+ylab("")+xlab("precision")

p_b_bar+{hp+bp+plot_layout(widths = c(.7,.3))}

aa<-all_out%>%dplyr::select(algo,precision,recall,threshold,qual)%>%group_by(algo,qual,threshold)%>%mutate(precision=mean(precision,na.rm=T),recall=mean(recall,na.rm=T))%>%melt(id.vars=c("algo","qual","threshold"))%>%unique()
pre<-aa[which(aa$qual=="restrictive"&aa$variable=="precision"),]%>%pull(value)
pre[8]/median(pre[1:7])

rec<-aa[which(aa$qual=="restrictive"&aa$variable=="recall"),]%>%pull(value)
median(rec[1:7])/rec[8]
...
all_out<-do.call(rbind,lapply(files,function(f){
  ...
  #all_counts$HLA<-sample(all_counts$HLA)
  #Summarize by allele and filter for peptides that are less than or equal to 5%
  p<-(true_counts[2]/sum(true_counts))
  algo_percentile<-c("mhcflurry_affinity_percentile","MixMHCpred","netMHC_affinity","netMHCpan_EL_affinity","netstab_affinity")
  F1_out<-do.call(rbind,lapply(1,function(i){
    thres="100% pocket"
    score<-sapply(algo_percentile,function(w){
      tmp<-cell_line_data%>%data.frame()%>%arrange(.[,w])%>%select(c("peptide","HLA",w,"ident"))%>%
        slice(-which(duplicated(peptide)))%>%slice(which(.[,w]<=quantile(.[,w],c(p))))
  ...
  cell_line_data$pickpocket_affinity<-50000^(1-cell_line_data$pickpocket_affinity)
  algo_BA<-c("pickpocket_affinity")
  F1_out_BA<-do.call(rbind,lapply(quantile(cell_line_data$pickpocket_affinity,c(p)),function(i){
    thres="100% pocket"
    score<-sapply(algo_BA,function(w){
      tmp<-cell_line_data%>%data.frame()%>%arrange(.[,w])%>%select(c("peptide","HLA",w,"ident"))%>%
        slice(-which(duplicated(peptide)))%>%slice(which(.[,w]<=quantile(.[,w],c(p))))
  ...
  F1_out_MHC<-lapply("mhcflurry_presentation_score",function(w){
    thres="100% pocket"
      tmp<-cell_line_data%>%data.frame()%>%arrange(desc(.[,w]))%>%select(c("peptide","HLA",w,"ident"))%>%
        slice(-which(duplicated(peptide)))%>%slice(which(.[,w]>=quantile(.[,w],c(1-p))))

      score=c(f=as.numeric(F1(tmp$ident,pos)),per=as.numeric(PPV(tmp$ident)),rec=as.numeric(table(tmp$ident)["target"]/pos))

    data.frame(algo=w,F1=as.numeric(score["f"]),PPV=as.numeric(score["per"]),recall=as.numeric(score["rec"]),threshold=thres)
  })[[1]]

  F1_ensemble<-do.call(rbind,lapply(c(.05),function(i){
    prob_threshold<-"100% pocket"
    df_all_proteins<-all_counts[order(all_counts$prob),]%>%slice(-which(duplicated(peptide)))%>%slice(which(prob<=i))
    data.frame(algo="EnsembleMHC",
               F1=as.numeric(F1(df_all_proteins$ident,pos)),
               PPV=as.numeric(PPV(df_all_proteins$ident)),
               recall=as.numeric(table(df_all_proteins$ident)["target"]/pos),
...
p_b_bar<-all_out%>%dplyr::select(algo,precision,recall,threshold)%>%group_by(algo,threshold)%>%mutate(precision=mean(precision,na.rm=T),recall=mean(recall,na.rm=T))%>%
  melt(id.vars=c("algo","threshold"))%>%ggplot(aes(x=algo,y = value,fill=variable))+geom_bar(stat="identity",position = position_dodge())+
  labs(fill="metric")+xlab("")+ylab("")+theme_pubclean()+theme(axis.text.x = element_text(angle = 90))+
  guides(fill=guide_legend(title.position = "top",title.hjust = .5))
...
bp<-all_out%>%group_by(combo_name)%>%summarise(F1=mean(F1))%>%ggplot(aes(y=combo_name,x = F1))+
  geom_bar(stat="identity")+theme_pubclean()+theme(axis.text.y = element_blank())+ylab("")+xlab("precision")
