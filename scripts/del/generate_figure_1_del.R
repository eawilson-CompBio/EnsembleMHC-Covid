dataset_path="~/Covid-19/EnsembleMHC-Covid19/datasets/"
...
#load the preclaculated benchmarking values
load(paste0(dataset_path,"P_sum_median_1000_boot.R"))

#read in the coronavirus predictions 
files<-list.files(paste0(dataset_path,"tumor_predictions"),pattern = "csv",full.names = T)

#calculate percentile score 
xx<-fread(paste0(dataset_path,"tumor_predictions/pres_percentile_mhcflurry_pred.out"))
...
score<-sapply(algo_percentile,function(w){
  ...
  #remove duplicate peptides as we are assigning each peptide to the highest scoring allele
...
#evaluate models that predicted IC50
...
#caculate values for MHCflurry presentation using derived percentile values
...
#calculate peptide probabilities for EnsembleMHC predrictions
#start by iterating through every unique HLA
pep_probs<-lapply(unique(cell_line_data$HLA),function(w){
  #create a tmp variable with that consists of the predictions for one allele
...
gg <- p_b_bar+{hp+bp+plot_layout(widths = c(.7,.3))}

ggsave(gg,filename = paste0(Ensemble_PATH,"/plots/main_figures/Figure_1.pdf"),height = 12,width = 15)