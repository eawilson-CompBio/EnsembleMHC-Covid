...
corona_data<-lapply(list.files(path="~/Covid-19/covid_predictions/",pattern = "csv",full.names = T),function(p){
  fread(p,stringsAsFactors = F)
  
})  

corona_data<-do.call(rbind,corona_data[which(sapply(corona_data,ncol)==14)])%>%data.frame()
#remove teh leading HLA-
corona_data$HLA<-str_remove(corona_data$HLA,"HLA-")
...
P_sum$value[which(P_sum$algo=="mhcflurry_presentation_score")]<-.5
...
pep_probs_ind<-lapply(unique(corona_data$HLA),function(w){
  ...
  prob_combo<-do.call(rbind,prob_list)%>%data.frame()%>%group_by(peptide)%>%merge(tmp[,c("peptide","gene","HLA")])
...
indv_metric<-lapply(algos,function(w){
  struct<-all_ind[which(all_ind$algo==w),]%>%slice(which(gene%in%c("E","N","M","S")))%>%group_by(HLA)%>%summarise(count=length(peptide))%>%mutate(pep_frac=count/sum(count))%>%mutate(source="Structural proteins")
  all<-all_ind[which(all_ind$algo==w),]%>%group_by(HLA)%>%summarise(count=length(peptide))%>%mutate(pep_frac=count/sum(count))%>%mutate(source="All proteins")
  rbind(all,struct)
...
#name list
names(allele_freq_list)<-unique(all_data_clean$Country.Region[which(all_data_clean$Confirmed>=1)])
...
ggplot(norm_df_Death,aes(days,correlation,group=confirmed))+geom_line(aes(color=confirmed))+
  ...
  xlab("normalized days")+ggtitle(names(indv_metric)[n])
...
ggsave(garg,filename = "~/Covid-19/plots/SI_figures/SI_individual_algorithms_PPV.pdf",height = 12,width = 15)